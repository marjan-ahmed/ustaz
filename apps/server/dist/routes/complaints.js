...
router.patch('/:id', requireAuth, async (req, res) => {
    ...
    if (!existing)
        return res.status(404).json({ error: 'Not found' });
    const role = req.authUser.role;
    if (role === 'STUDENT' && existing.studentId !== req.authUser.id)
        return res.status(403).json({ error: 'Forbidden' });
    const allowed = {};
    ...
    if (typeof req.body.isPublic === 'boolean')
        allowed.isPublic = req.body.isPublic;
    if (role !== 'STUDENT') {
        if (typeof req.body.status === 'string')
            allowed.status = req.body.status;
        if (typeof req.body.assignedToId === 'string')
            allowed.assignedToId = req.body.assignedToId;
    }
...
router.post('/:id/comments', requireAuth, async (req, res) => {
    ...
    if (!complaint)
        return res.status(404).json({ error: 'Not found' });
    if (req.authUser.role === 'STUDENT' && complaint.studentId !== req.authUser.id)
        return res.status(403).json({ error: 'Forbidden' });
