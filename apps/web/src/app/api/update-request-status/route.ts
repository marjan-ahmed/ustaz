    ...
    switch (action) {
      ...
      case 'completed':
        if (sr.status === 'arrived') {
          const startResp = await supabase.rpc('start_service', {
            p_request_id: requestId,
            p_provider_id: user.id,
          });
          const startRow = Array.isArray(startResp.data) ? startResp.data[0] : startResp.data;
          if (startResp.error || !startRow || startRow.success === false) {
            resp = startResp;
            break;
          }
        }
